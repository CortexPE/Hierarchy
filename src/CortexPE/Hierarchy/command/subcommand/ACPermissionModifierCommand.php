...
abstract class ACPermissionModifierCommand extends HierarchySubCommand implements FormedCommand {
	...
	protected function prepare(): void {
		$this->registerArgument(0, new TargetEnumArgument("targetType", false));
		$this->registerArgument(1, new MemberArgument("target", false));
		$this->registerArgument(1, new RoleArgument("target", false));
		$this->registerArgument(2, new PermissionArgument("permission", false));
	...
	public function onRun(CommandSender $sender, string $aliasUsed, array $args): void {
		...
		/** @var BaseMember|Role|null $target */
		$target = $args["target"];
				...
				function (Player $player, bool $choice): void {
					if($choice) {
						...
						foreach($this->roleManager->getRoles() as $role) {
							if($role->isDefault() || !$this->doHierarchyPositionCheck($role)) {
								continue;
							}
